...
- **A URL that answers isn't necessarily the URL on file.** Every other rule here asks whether a request succeeded; this one asks where. A redirect to a different site, a different section, or a bare home page means the stored address is stale — and because it answers `200`, no status check will ever say so. The Tanzania chamber's domain lapsed and redirected to an online-gambling site while being reported healthy every month. For a `RSS Feed` cell the test is stricter: the body has to be a feed *with entries*, since a valid empty feed passes every other check. None of these are removal candidates — a page that moved is a stale cell, not a dead source.
